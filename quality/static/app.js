...
function addAnalyticsButton(id){
  ...
  button.onclick=()=>renderAnalytics(id);
  const hero=$('.job-hero');if(!hero)return;
  const stack=document.createElement('div');stack.className='job-action-stack';
  const actions=hero.querySelector('.job-hero-actions');
  if(actions){actions.parentNode.insertBefore(stack,actions);stack.append(actions)}else hero.append(stack);
  stack.append(button);
}

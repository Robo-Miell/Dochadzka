...
async function renderNewRecord(extra=null){
  ...
  function updateJob(){
    ...
    if(j.norm_mode==='time') $('#normInputWrap').innerHTML=`<label>Pracovný čas / Working time (HH:MM)<input id="rOperatorTime" placeholder="hh:mm" inputmode="text" value="${editRecord?.work_time_seconds?esc(String(Math.floor(editRecord.work_time_seconds/3600)).padStart(2,'0')+':'+String(Math.floor((editRecord.work_time_seconds%3600)/60)).padStart(2,'0')):''}"><small>Napr. 01:20. Zadané 1:20 sa automaticky upraví na 01:20.</small></label>`;
...
function multiSelectMarkup(id,label,options){return `<div class="multi-field"><span class="field-label">${esc(label)}</span><details class="multi-select" id="${id}"><summary><span class="multi-summary">Všetky</span><span>▾</span></summary><div class="multi-popover"><input class="multi-search" type="search" placeholder="Hľadať…" aria-label="Hľadať ${esc(label)}"><div class="multi-options">${options.map(o=>`<label><input type="checkbox" value="${esc(o.value)}"><span>${esc(o.label)}</span></label>`).join('')||'<div class="muted">Žiadne možnosti</div>'}</div></div></details></div>`}
function bindMulti(id){const d=$('#'+id);if(!d)return;const upd=()=>{const c=[...d.querySelectorAll('input:checked')];d.querySelector('.multi-summary').textContent=c.length?`${c.length} vybrané`:'Všetky'};d.querySelectorAll('input[type=checkbox]').forEach(x=>x.onchange=upd);d.querySelector('.multi-search').oninput=e=>{const q=searchText(e.target.value);d.querySelectorAll('.multi-options label').forEach(l=>l.hidden=!searchText(l.textContent).includes(q))};upd()}
